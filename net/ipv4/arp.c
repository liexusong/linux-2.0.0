...
static __inline__ void arpd_update(struct arp_table * entry)
{
    if (arpd_not_running)
        return;
    arpd_send(ARPD_UPDATE, entry->ip, entry->dev, entry->ha, entry->last_updated);
}
...
#endif
                arp_send(ARPOP_REQUEST, ETH_P_ARP, paddr, dev,
          ...
          unsigned char *target_hw)
{
    ...
    skb = alloc_skb(sizeof(struct arphdr)
                        + 2*(dev->addr_len+4)
                        + dev->hard_header_len,
                    GFP_ATOMIC);
 ...
 */
    ...
    struct arphdr *arp = (struct arphdr *)skb->h.raw; // ARP头部
    unsigned char *arp_ptr = (unsigned char *)(arp+1);
    unsigned char *sha, // 源mac地址
                  *tha; // 目标mac地址
    u32 sip,            // 源IP地址
        tip;            // 目标IP地址
 ...
 */
        if (tip != dev->pa_addr) { // 如果目标IP地址不是设备对应的IP地址
 ...
 */
            ...
            if (proxy_entry &&
            	(proxy_entry->mask || ((dev->pa_addr ^ tip) & dev->pa_mask)))
            {
