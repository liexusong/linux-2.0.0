...
int ip_rcv(struct sk_buff *skb, struct device *dev, struct packet_type *pt)
{
    ...
    if (skb->len < sizeof(struct iphdr)                    // 数据长度是否正确
        || iph->ihl < 5
        || iph->version != 4
    ...
    skb_trim(skb, ntohs(iph->tot_len)); // 缩小尾部长度到len(去掉链路层的尾部)
...
#define CONFIG_IP_ALWAYS_DEFRAG 1
    ...
    if (iph->frag_off) {                  // 处理IP分片
        ...
        if (is_frag) { // 如果是分片
            ...
            skb = ip_defrag(iph,skb,dev); // IP分片处理
            if (skb == NULL)
                return 0;
...
#ifdef CONFIG_FIREWALL // 如果配置了防火墙
...
#endif
    {
             ...
             ipprot = (struct inet_protocol *)ipprot->next)
        {
            ...
            if (ipprot->protocol != iph->protocol) // 下一个协议
                continue;
