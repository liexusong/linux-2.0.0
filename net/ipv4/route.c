...
static struct fib_node *fib_lookup_local(__u32 dst)
{
    ...
    for (fz = fib_zone_list; fz; fz = fz->fz_next) {
        ...
        for ( ; f; f = f->fib_next) {
            if ((dst ^ f->fib_dst) & fz->fz_mask) // no matched?
                continue;
            if (!(f->fib_info->fib_flags & RTF_GATEWAY)) // 不是一个网关, 返回这个路由
                return f;
...
static struct fib_node *fib_lookup(__u32 dst)
{
    ...
    for (fz = fib_zone_list; fz; fz = fz->fz_next) {
        ...
        for ( ; f; f = f->fib_next) {
            if ((dst ^ f->fib_dst) & fz->fz_mask) // 不匹配
                continue;
            return f;
        }
    }
                        ...
                        __u32 mask)
{
    ...
    while ((f = *fp) != NULL) {
        ...
        *fp = f->fib_next;
        ...
        ip_netlink_msg(RTMSG_DELROUTE, dst, gtw, mask,
                       flags, metric, fi->fib_dev->name);
...
fib_create_info(__u32 gw, struct device * dev,
...
fib_add_1(short flags, __u32 dst,
          __u32 mask, __u32 gw,
          struct device *dev,
          unsigned short mss,
            ...
            (gw || f1->fib_info->fib_dev == dev))
        {
            ...
            *fp = f1->fib_next;
            if (fib_loopback == f1)
                fib_loopback = NULL;
            sti();
            ip_netlink_msg(RTMSG_DELROUTE, dst, gw, mask,
            	           flags, metric, f1->fib_info->fib_dev->name);
            fib_free_node(f1);
            fz->fz_nent--;
            break;
        }
...
struct rtable *ip_rt_route(__u32 daddr, int local)
{
    ...
    for (rth = ip_rt_hash_table[ip_rt_hash_code(daddr)^local];
         rth;
         rth = rth->rt_next)
    {
        if (rth->rt_dst == daddr) {
...
int ip_rt_kill(struct rtentry *r)
{
     ...
     */
    err = rt_del((__u32)trg->sin_addr.s_addr,
                 (__u32)msk->sin_addr.s_addr,
                 dev,
                 (__u32)gtw->sin_addr.s_addr,
                 r->rt_flags,
                 r->rt_metric - 1);
