    ...
    __u32 saddr, struct inet_protocol *protocol)
{
    ...
    if (code < 13) {
        if (icmp_err_convert[code].fatal
    ...
    int len, int nonblock, int flags)
{
    ...
    while (--iovlen >= 0) {
         ...
         */
            ...
            tmp = sk->prot->build_header(skb, sk->saddr, sk->daddr,
                                         &dev, IPPROTO_TCP, sk->opt,
                                         skb->truesize, sk->ip_tos,
                       ...
                       int len, int nonblock, int flags)
{
    ...
    lock_sock(sk);
    retval = do_tcp_sendmsg(sk, msg->msg_iovlen,
                            msg->msg_iov, len, nonblock, flags);
...
static int tcp_connect(struct sock *sk, struct sockaddr_in *usin, int addr_len)
{
    ...
    tmp = sk->prot->build_header(buff, sk->saddr, sk->daddr, &dev,
                                 IPPROTO_TCP, sk->opt, MAX_SYN_SIZE,
