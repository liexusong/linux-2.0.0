...
get_tcp_sock(u32 saddr, u16 sport, u32 daddr,
             u16 dport, u32 paddr, u16 pport)
{
    struct sock * sk;

    sk = (struct sock *)th_cache_sk;
    if (!sk
        || saddr != th_cache_saddr
        || daddr != th_cache_daddr
        || sport != th_cache_sport
        || dport != th_cache_dport)
    {
...
static void
bad_tcp_sequence(struct sock *sk, struct tcphdr *th, u32 end_seq, struct device *dev)
{
    if (th->rst)
        return;
    ...
    if (sk->state == TCP_SYN_SENT || sk->state == TCP_SYN_RECV) {
        tcp_send_reset(sk->saddr, sk->daddr, th, sk->prot,
                       NULL,dev, sk->ip_tos, sk->ip_ttl);
        return;
    }
...
static void tcp_conn_request(struct sock *sk, struct sk_buff *skb,
                             u32 daddr, u32 saddr, struct options *opt,
                             struct device *dev, u32 seq)
{
    struct sock *newsk;
    struct tcphdr *th;
    struct rtable *rt;
    ...
    if (opt && opt->optlen) {
        sk->opt = (struct options*)
                        kmalloc(sizeof(struct options)+opt->optlen, GFP_ATOMIC);
        if (!sk->opt) {
    ...
    newsk->mtu = min(newsk->mtu,
                     dev->mtu - sizeof(struct iphdr) - sizeof(struct tcphdr));
                    ...
                    unsigned long saddr, unsigned int len)
{
    ...
    if(sk->shutdown & RCV_SHUTDOWN) {
        ...
        if(skb->len) {  /* We don't care if it's just an ack or a keepalive/window probe */
        ...
        struct inet_protocol *protocol)
{
    ...
    if (!redo) {
        ...
        skb_pull(skb, skb->h.raw-skb->data); // 丢弃TCP头部
    ...
    if (sk->state != TCP_ESTABLISHED) { /* Skip this lot for normal flow */
        ...
        if (sk->state == TCP_LISTEN) { // 处理 listen 状态的 socket
            ...
            if (th->rst
                || !th->syn
                || th->ack
                || (r=ip_chk_addr(daddr) == IS_BROADCAST || r == IS_MULTICAST))
#else
            if (th->rst
                || !th->syn
                || th->ack
        ...
        if (sk->state == TCP_SYN_SENT) {
            ...
            if (th->ack) {
                 ...
                 */
                if (after(skb->ack_seq, sk->sent_seq)
                    || before(skb->ack_seq, sk->rcv_ack_seq))
                {
...
#define BSD_TIME_WAIT
            ...
            && !th->rst)
        {
            ...
            sk = get_sock(&tcp_prot, th->dest, saddr, th->source,
