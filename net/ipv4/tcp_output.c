...
int tcp_new_window(struct sock *sk)
{
    ...
    unsigned long minwin, maxwin; // 最小窗口与最大窗口
...
{
    int size;
    struct tcphdr *th = skb->h.th; // TCP头部指针
    ...
    if (size == sizeof(struct tcphdr)) { // 如果只有TCP头部
    ...
    skb->seq = ntohl(th->seq); // 当前序列号
    skb->end_seq = skb->seq + size - 4*th->doff; // 此数据包之后的序列号
    ...
    if (after(skb->end_seq, sk->window_seq) // skb->end_seq > sk->window_seq ? 1 : 0
    ...
    else
    {
        ...
        tcp_send_check(th, sk->saddr, sk->daddr, size, skb); // 计算校验和
        ...
        sk->prot->queue_xmit(sk, skb->dev, skb, 0); // ip_queue_xmit()
                    ...
                    int tos, int ttl)
{
    ...
    tmp = prot->build_header(buff, saddr, daddr, &ndev, IPPROTO_TCP,
                             opt, sizeof(struct tcphdr), tos, ttl, NULL);
    if (tmp < 0) {
        buff->free = 1;
...
void tcp_send_synack(struct sock * newsk, struct sock * sk, struct sk_buff * skb)
{
    ...
    tmp = sk->prot->build_header(buff, newsk->saddr, newsk->daddr,
                                 &ndev, IPPROTO_TCP, newsk->opt,
                                 MAX_SYN_SIZE, sk->ip_tos, sk->ip_ttl,
                                 &newsk->ip_route_cache);
...
void tcp_write_wakeup(struct sock *sk)
{
        ...
        && (skb = skb_peek(&sk->write_queue)))
    {
        ...
        struct tcphdr *th;
        struct tcphdr *nth;
        unsigned long win_size, real_size;
#if 0
        ...
        real_size = win_size + th->doff * 4
                   + (iph->ihl << 2) + sk->prot->max_header + 15;

        buff = sock_wmalloc(sk, real_size, 1, GFP_ATOMIC);
         if (buff == NULL)
             return;
        ...
        tmp = sk->prot->build_header(buff, sk->saddr, sk->daddr,
                                      &dev, IPPROTO_TCP, sk->opt,
                                      buff->truesize, sk->ip_tos,
