			 ...
			 __u32 saddr, struct inet_protocol *protocol)
{
	...
	if (type == ICMP_SOURCE_QUENCH) {	/* Slow down! */
		if (sk->cong_window > 1)
					...
					__u32 saddr, int noblock)
{
	...
	if (rt & MSG_PROXY) {
		 ...
		 */
		struct sockaddr_in *sinfrom = (struct sockaddr_in *) sin->sin_zero;
...
static int udp_sendto(struct sock *sk, const unsigned char *from,
...
int udp_chkaddr(struct sk_buff *skb)
{
	...
	sk = get_sock(&udp_prot, uh->dest, iph->saddr,
				  uh->source, iph->daddr, 0, 0);
...
int udp_rcv(struct sk_buff *skb, struct device *dev,
			...
			__u32 saddr, int redo, struct inet_protocol *protocol)
{
	...
	if (uh->check &&
		(((skb->ip_summed == CHECKSUM_HW)
				&& udp_check(uh, len, saddr, daddr, skb->csum))
	...
	if (addr_type == IS_BROADCAST || addr_type==IS_MULTICAST) {
		...
		sk = get_sock_mcast(
					udp_prot.sock_array[ntohs(uh->dest)&(SOCK_ARRAY_SIZE-1)],
		...
		if (sk) {
			do {
				...
				sknext = get_sock_mcast(sk->next, uh->dest,
